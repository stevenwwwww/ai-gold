...
export async function generateSummary(rawText: string, reportId?: string): Promise<ReportSummary> {
  const { summaryMaxInputChars } = await import('../config').then(m => m.config)
  let truncated = rawText.slice(0, summaryMaxInputChars)

  if (rawText.length > summaryMaxInputChars && reportId) {
    try {
      const { getChunks } = await import('./rag/vectorStore')
      const chunks = getChunks(reportId)
      if (chunks.length > 0) {
        truncated = chunks.map((c) => c.text).join('\n\n').slice(0, summaryMaxInputChars)
        console.log(`[Summary] 使用 RAG 分块进行摘要`)
      }
    } catch { /* 降级为截断 */ }
  }
